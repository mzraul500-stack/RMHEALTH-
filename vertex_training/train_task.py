"""
RMHealth Triage Classifier v2 — Vertex AI Custom Training Script.

Genera dataset sintético clínico (NEWS2/AHA/ADA/qSOFA), entrena
GradientBoostingClassifier con pesos correctos para FC/SpO2/Presión,
evalúa métricas por clase y guarda el modelo en Cloud Storage.
...
log = logging.getLogger("RMHealth.TrainV2")
...
METRICS_PATH  = f"gs://{BUCKET_NAME}/metrics/triage_metrics_v2.json"
...
def generate_dataset(n_per_class: int = 2500, seed: int = 42) -> pd.DataFrame:
    ...
    # Glucosa puede estar elevada sin que eso cambie la clase.
    for _ in range(n_per_class):
        fc   = rng.integers(60, 101)
        spo2 = rng.integers(95, 101)
        tas  = rng.integers(90, 130)
        tad  = rng.integers(60, 85)
        gluc = rng.integers(70, 200)    # rango amplio — NO define la clase
    ...
    # FC o SpO2 son el discriminador primario.
    for i in range(n_per_class):
        ...
        elif subtype == 1: # Hipoxemia leve
            fc   = rng.integers(60, 105)
            spo2 = rng.integers(91, 95)
            tas  = rng.integers(90, 135)
            tad  = rng.integers(60, 85)
        elif subtype == 2: # Hipertensión Etapa 1-2
            fc   = rng.integers(60, 105)
            spo2 = rng.integers(93, 100)
        ...
        else:              # Combinación leve: FC+TAS ligeramente altos
    ...
    # ── ALTO: NEWS2 5-6  ─────────────────────────────────────
    # Signos vitales con alteración significativa.
    # Casos combinados cardiopulmonares obligatorios.
    for i in range(n_per_class):
        subtype = i % 6
...
def run_clinical_cases(model, le) -> list:
    """
    Ejecuta los 5 casos clínicos de validación de Raúl.
    Devuelve lista con resultados y si pasan el criterio.
    """
    cases = [
        ...
        {"name": "Caso D", "fc": 72,  "spo2": 98, "tas": 118, "tad": 76,
         ...
         "expected_min": ["BAJO"]},
        {"name": "Caso E (RAÚL)", "fc": 118, "spo2": 89, "tas": 140, "tad": 95,
         "gluc": 90, "temp": 36.5, "age": 50,
         "expected_min": ["CRITICO"]},
    ]
...
def main():
    # ── 1. Generar dataset ──────────────────────────────────────
    log.info("=== RMHealth Triage Classifier v2 — Training Job ===")
    df = generate_dataset(n_per_class=2500, seed=42)
    ...
    if metrics["deploy_approved"] and all_passed:
        ...
        # 5a. Guardar en nuestra ruta canónica de GCS (para descarga directa)
        model_gcs_uri = f"gs://{BUCKET_NAME}/models/triage_classifier_v2.joblib"
        upload_to_gcs(local_model, model_gcs_uri)
        log.info("Modelo subido a ruta canonica: %s", model_gcs_uri)
